...
select name from tracks t 
	--where t.name ~* '( my |^my|my$|^мой|мой$| мой )'
...
--Все исполнители, которые не выпустили альбомы в 2020 году
		select m.nickname from musicians m 
			where m.id not in (
				select m2.id from musicians m2 
				inner join musicians_to_albums mta on m2.id =mta.musician_id
...
--Названия альбомов, в которых присутствуют исполнители более чем одного жанра
	select distinct a."name"  from albums a
		...
		inner join genres g on g.id = gtm.genre_id
			group by a."name", gtm.musician_id 
				having count(*) > 1;
